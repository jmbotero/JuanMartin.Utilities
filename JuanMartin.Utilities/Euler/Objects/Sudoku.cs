...
namespace JuanMartin.Utilities.Euler.Objects
{
    ...
    public class Sudoku
    {
        ...
        private Dictionary<Tuple<int, int>, Tuple<int, int>> _coordinates;
        ...
        public Sudoku(string name, List<List<int>> rows = null)
        {
            ...
            {
                var col = UtilityList.ListRepeatedElements<int>(0, GridHeight);
                Columns.Add(col);
            }
        ...
        {
            var foundSolution = BackTrack(GetEmptyCells());
        ...
        public bool SetGridValue(int i, int j, int value, bool overwrite = false, bool blockBased = false)
        {
            var update = false;
            var cellCoordinates = new Tuple<int, int>(i, j);
            var blockCoordinates = _coordinates[cellCoordinates];
            ...
            if (blockBased)
            {
                blockI = i;
                blockJ = j;
                i = blockCoordinates.Item1;
                j = blockCoordinates.Item2;
            }
            else
            {
                blockI = blockCoordinates.Item1;
                blockJ = blockCoordinates.Item2;
            }
        ...
        {
            var rowContains = Rows[i].Contains(value);
            var columnContains = Columns[j].Contains(value);

            if (BlockHeight != GridHeight)
            {
                var cellCoordinates = new Tuple<int, int>(i, j);

                if (_coordinates.ContainsKey(cellCoordinates))
                {
                    var blockCoordinates = _coordinates[cellCoordinates];
                    var blockContains = Blocks[blockCoordinates.Item1].Contains(value);
                    return rowContains | columnContains | blockContains;
                }
                else
                    return false;
            }
        ...
        private void SetBlocks()
        {
            ...
            if (count != -1)
            {
                for (int i = 0; i < GridHeight; i++)
                {
                    var block = UtilityList.ListRepeatedElements<int>(0, count);
                    Blocks.Add(block);
                }
            }
            // second populate block list
            foreach (KeyValuePair<Tuple<int, int>, Tuple<int, int>> entry in _coordinates)
            {
        ...
        {
            _coordinates = new Dictionary<Tuple<int, int>, Tuple<int, int>>();
            ...
            if (BlockHeight != GridHeight)
            {
                ...
                for (int i = 0; i < GridHeight; i++)
                {
                    ...
                    for (int j = 0; j < GridHeight; j++)
                    {
                        var blockI = GetHorizontalShift(i) * BlockHeight + GetHorizontalShift(j);
                        var blockJ = GetVerticalShift(i) * BlockHeight + GetVerticalShift(j);
                        var cellcoordinates = new Tuple<int, int>(i, j);
                        var blockcoordinates = new Tuple<int, int>(blockI, blockJ);
                        _coordinates.Add(cellcoordinates, blockcoordinates);
                    }
                }
            }
            else
        ...
        private bool BackTrack(List<Tuple<int, int>> emptyCells)
        {
            ...
            var i = emptyCell.Item1;
            var j = emptyCell.Item2;
            ...
            for (int value = 1; value < GridHeight + 1; value++)
            {
                ...
                if (check)
                {
                    ...
                    // backtracking
                    if (BackTrack(emptyCells.GetRange(1, emptyCells.Count - 1)))
                        return true;
